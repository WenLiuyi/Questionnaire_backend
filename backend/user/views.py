...
def delete_filled_qs(request):
    if(request.method=='POST'):
        try:
            ...
            submission.delete()
            return JsonResponse({'success': 'Submission deleted successfully'})  

        except json.JSONDecodeError: 
            return JsonResponse({'error': 'Invalid JSON body'}, status=400)
        except ProtectedError:  
            return JsonResponse({'error': 'Cannot delete submission due to protection'}, status=409)  # 使用409 Conflict来表示无法删除
